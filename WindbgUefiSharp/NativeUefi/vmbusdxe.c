...
//	BOOLEAN vmbus_channel_request_offer_response_show = TRUE;
	...
	else if (hdr->msgtype == CHANNELMSG_OFFERCHANNEL)
	{
		...
		if (vmbus_channel_request_offer_response_show)
		{
			...
			KdpDprintf(L"vmbus_channel_request_offer_response!msgtype:=>%08x,child_relid:=>%08x,connection_id:=>%08x\r\n", offer->header.msgtype, offer->child_relid,offer->connection_id);
...
BOOLEAN  channel_child_relid_synic_event_page_sint()
{
	volatile PHV_SYNIC_EVENT_FLAGS  synic_event_page_sint = (PHV_SYNIC_EVENT_FLAGS)synic_event_page + vmbus_sint;
	return _bittestandreset64((__int64*)synic_event_page_sint, gpipedev.child_relid);
...
VOID NTAPI  SyncRingBufferRoot(UINT64 bufpage, UINT32 sig_event);
...
EFI_STATUS NTAPI HvVmbusServiceDxeInitialize()
{
	...
	struct hv_ring_buffer_info* outring_info = &gpipedev.send_buf;
	u32 next_write_location = hv_get_next_write_location(outring_info);
	
	SyncRingBufferRoot(gpipedev.send_buf.bufpage, gpipedev.sig_event);
