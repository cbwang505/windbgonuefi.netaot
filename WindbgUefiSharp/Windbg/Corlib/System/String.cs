...
namespace System
{
    public sealed unsafe class String
    {
        ...
        public string Append(string line)
        {
            ...
            return string.Concat(this, line); ;
        }
        public string Append(char line)
        {

            return string.Concat(this, line.ToString()); ;
        }
        ...
        public string PadLeft(int totalWidth, char paddingChar)
        {
            ...
            int remain = totalWidth - this.Length;
            string ret = "";
            string retref = this;
            // string ret = new string(paddingChar, remain);
        ...
        public string Trim()
        {
            ...
            int auxidx = 0;
            char* newp = stackalloc char[retlen];
            for (int i = 0; i < this.Length; i++)
            {
                if (auxidx < retlen)
                {
            ...
            return new string(newp, 0, retlen);
        ...
        public unsafe char[] ToCharArray()
        {
            ...
            if (length > 0)
            {
                ...
                fixed (char* dmem = charArray)
                    StartupCodeHelpers.memcpy((byte*)dmem, (byte*)smem, length * sizeof(char));
            }
            return charArray;
        }
...
#pragma warning disable CS0824 // Constructor is marked external
        ...
        public extern unsafe String(char buf, int length);

#pragma warning restore CS0824 // Constructor is marked external
        public static bool IsASCII(char cd)
        {

            return cd != 0 && ((cd < 127) && (cd > 31) && (cd != 92) && (cd != 34) &&
                               (cd != 37));
        }
        ...
        public static unsafe string FromASCII(IntPtr ptr)
        {
            int length = 0;
            byte* pCurrentstring = (byte*)ptr;
            while (true)
            {
                char cd = (char)*pCurrentstring;
                if (IsASCII(cd))
                {
                    pCurrentstring++;
                    length++;
        ...
        private static unsafe string Ctor(char* ptr, int index, int length)
        {
            ...
            {
                StartupCodeHelpers.memcpy((byte*)c, (byte*)start, (ulong)length * sizeof(char));
                c[length] = '\0';
            }
        ...
        private static unsafe string Ctor(char chr, int length)
        {
            string s = "";
            for (int i = 0; i < length; i++)
            {
